...
// on-dark colour). Shipped as an `<img src>` it is an isolated document where that fixed light fill
// paints on every theme, leaving it near-invisible on the light theme. Inlined with
// `fill="currentColor"` it takes the surrounding label colour instead (black-on-light,
// white-on-dark), exactly as grok's mark does.

/** The mark's own aspect box (24×24, GitHub's octicon grid). */
export const COPILOT_MARK_VIEWBOX = '0 0 24 24'

/** Two subpaths: the head/body glyph and the two eyes. Both painted in `currentColor`. */
export const COPILOT_MARK_PATHS = [
...
export function createCopilotMarkSvg(size: number, className: string): SVGSVGElement {
  ...
  svg.setAttribute('viewBox', COPILOT_MARK_VIEWBOX)
  svg.setAttribute('fill', 'currentColor')
  svg.setAttribute('aria-hidden', 'true')
  svg.setAttribute('class', className)
  for (const d of COPILOT_MARK_PATHS) {
