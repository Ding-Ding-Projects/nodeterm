...
describe('licenseSentence — a keygen license that read cleanly', () => {
  it('reports usage against the cap and names phones as devices', () => {
    ...
    expect(s).toBe('2 of 3 devices in use — this PC and each paired phone counts as a device.')
    ...
    expect(licenseSentence(keygen({ used: 1, seats: 5 }))).toBe(
      '1 of 5 devices in use — this PC and each paired phone counts as a device.'
    )
  })
...
describe('licenseSentence — a read that FAILED', () => {
  ...
  it('does not promise Pro is unaffected when the server said the license is inactive', () => {
    ...
    expect(s).toBe(
      'This license is not active, so there is no key or device count to show — and Pro on this PC will stop when its current entitlement expires.'
    )
  ...
  it('does not promise Pro is unaffected when this PC was refused', () => {
    const s = licenseSentence({ key: null, used: 0, seats: 0, source: null, error: 'unauthorized' })
    expect(s).toBe(
      'This PC is not authorized on this license, so the key and device count are unavailable — and Pro here may stop when its current entitlement expires.'
    )
    expect(s).not.toContain('unaffected')
...
describe('canUseKeyElsewhere', () => {
  ...
  it('is false AT the cap — the activation it invites would be refused', () => {
    ...
    // the key on another PC, where the server answers seat_limit. Advice that cannot work.
...
describe('releaseFailureSentence', () => {
  ...
  it('states that nothing was released when the server ANSWERED and refused', () => {
    expect(releaseFailureSentence('unauthorized')).toBe(
      'Could not release the other devices — this PC is not authorized on this license. Nothing was released.'
    )
...
describe('activationErrorSentence', () => {
  ...
  it('turns seat_limit into the remedy, and points it at a machine that HAS Pro', () => {
    ...
    expect(s).toBe(
      'This license already has all its devices in use. On a PC (or phone) where Pro is active, open Settings → License and choose “Release other devices”, then activate here again.'
    )
