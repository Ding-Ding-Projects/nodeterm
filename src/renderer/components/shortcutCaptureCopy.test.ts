...
describe('shortcutCaptureCopy', () => {
  it('names the command and the chord that took the key', () => {
    const copy = shortcutCaptureCopy('app.commandPalette')
  ...
  it('spells the chord with Windows Control notation', () => {
    expect(shortcutCaptureCopy('app.commandPalette')?.body).toContain('Ctrl+K')
  })

  it('follows a remap, so the notice quotes the chord the user actually pressed', () => {
    setKb({ 'app.commandPalette': ['Ctrl+Shift+P'] })
    expect(shortcutCaptureCopy('app.commandPalette')?.body).toContain('Ctrl+Shift+P')
  })
  ...
  it('is null for an unbound command', () => {
    expect(shortcutCaptureCopy('canvas.fitAll')).toBeNull()
  })

  it('is null for a DISABLED command', () => {
    setKb({ 'app.commandPalette': [] })
    expect(shortcutCaptureCopy('app.commandPalette')).toBeNull()
  })
  ...
  it('is null for an id the registry does not know', () => {
    expect(shortcutCaptureCopy('bogus.command' as CommandId)).toBeNull()
  })
})
