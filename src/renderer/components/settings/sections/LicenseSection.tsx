...
export function LicenseSection({ isActive }: { isActive: boolean }): React.JSX.Element {
  ...
  // "offline" and "this PC is not authorized" owe the user different sentences, and neither of
  ...
  return (
    ...
    >
            ...
            {detail ? (
              <>
                ...
                {canUseKeyElsewhere(detail) ? (
                  <p className="text-sm text-muted">
                    To use Pro on another PC, open Settings → License there and paste this key.
                  </p>
                ) : null}
      ...
      {confirming === 'release' ? (
        <ConfirmDialog
          message="Release every other device on this license? Your other PCs and every paired phone lose Pro until they are activated again — this PC keeps it. Devices can only be released once every 30 days."
      ...
      {confirming === 'deactivate' ? (
        <ConfirmDialog
          ...
          message={
            keyOnFile
              ? 'Deactivate Pro on this PC? Copy your license key first — it is shown here only while Pro is active, and you need it to activate again.'
              : 'Deactivate Pro on this PC? Pro features stop here until this device is activated again.'
          }
