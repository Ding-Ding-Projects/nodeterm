...
export function electronPlatform(): ElectronPlatform {
  ...
  return {
    ...
    // Seal / unseal node secrets at rest with the OS credential vault. Byte-in byte-out, mirroring #167's
    ...
    // CorePlatform). If the credential vault is unavailable safeStorage throws, which node-auth-secret.ts
