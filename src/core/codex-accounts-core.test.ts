...
describe('managed Codex account paths', () => {
  ...
  it('keeps the managed daemon socket below Linux SUN_LEN', () => {
    const userData = 'C:/Users/example/AppData/Local/node-terminal'
    const accountId = 'be28d3d4-c18c-430c-a257-ae550d3dd7ed'
    expect(Buffer.byteLength(codexSocketForAccount(userData, accountId))).toBeLessThan(108)
  })
})
...
describe('legacy home migration (real fs, fail closed)', () => {
  it('moves an existing long managed home to its deterministic short home', () => {
    ...
    try {
      const userData = path.join(fixture, 'AppData', 'Local', 'node-terminal')
